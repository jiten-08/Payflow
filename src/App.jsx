...
function App() {
  ...
  const viewProps = { stocks, bySymbol, wallet, setWallet, portfolio, setPortfolio, orders, setOrders, watchlist, setWatchlist, settings, setSettings, paymentMethods, setPaymentMethods, query, setQuery, setDrawer, setTrade, setAddMoney, setToast, holdings, summary };
...
function Orders({ orders }) { const exportCsv = () => { const rows = [["Type", "Stock", "Date", "Time", "Price", "Qty", "Status"], ...orders.map(o => [o.side, o.symbol, o.date, o.time, o.price, o.qty, o.status])]; const blob = new Blob([rows.map(r => r.join(",")).join("\n")], { type: "text/csv" }); const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "pay-flow-orders.csv"; a.click(); }; return <><PageTitle title="Orders" subtitle="Executed demo trades, newest first." action={<button className="secondary" onClick={exportCsv}><i className="bi bi-download" /> Export CSV</button>} /><Panel><OrdersTable orders={orders} /></Panel></>; }
...
function OrdersTable({ orders }) { if (!orders.length) return <Empty text="No orders yet." />; return <div className="table-wrap"><table><thead><tr><th>Type</th><th>Stock</th><th>Date</th><th>Time</th><th>Price</th><th>Qty</th><th>Status</th></tr></thead><tbody>{orders.map(o => <tr key={o.id}><td className={o.side === "Buy" ? "up" : "down"}>{o.side}</td><td>{o.symbol}</td><td>{o.date}</td><td>{o.time}</td><td>{rupee(o.price)}</td><td>{o.qty}</td><td><span className="executed">{o.status}</span></td></tr>)}</tbody></table></div>; }
